...
Synthetic/local records only; no publication or live ingestion.
"""
...
class TraceableClaim:
    ...
    retrieved_at: str
    def validate(self):
        if any(not isinstance(v,str) or not v.strip() for v in (self.claim_id,self.text,self.retrieved_at)):
        ...
        if not self.evidence: raise ValueError('missing_evidence_trace')
        for ref in self.evidence:
            if not isinstance(ref.evidence_id,str) or not ref.evidence_id.strip() or not isinstance(ref.relation,EvidenceRelation):
                raise ValueError('invalid_evidence_trace')
    def audit_path(self):
        self.validate()
        return {'claim_id':self.claim_id,'source_ids':self.source_ids,
